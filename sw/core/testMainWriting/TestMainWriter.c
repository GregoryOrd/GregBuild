...
int writeTestsToTestMain(TestFileList *testFiles, SourceFileList *sourceFiles,
...
void addTestMainCFunctionPointerDefinitions(char *main,
                                            TestFileList *testFiles) {
  for (int fileIndex = 0; fileIndex < testFiles->size; fileIndex++) {
    TestFile *file = &testFiles->files[fileIndex];
    addTestMainCFunctionPointerDefinitionsForSpecificFile(
        main, file->numTestCases, file->cases);
  }
}
...
void addTestMainCFunctionPointerCalls(char *main, TestFileList *testFiles) {
  for (int fileIndex = 0; fileIndex < testFiles->size; fileIndex++) {
    TestFile *file = &testFiles->files[fileIndex];
    addTestMainCFunctionPointerCallsForSpecificFile(main, file->numTestCases,
                                                    file->cases);
  }
}

void addTestMainCFunctionPointerDefinitionsForSpecificFile(char *main,
                                                           int numTests,
                                                           TestCase *cases) {
...
void addTestMainCFunctionPointerCallsForSpecificFile(char *main, int numTests,
                                                     TestCase *cases) {
  for (int i = 0; i < numTests; i++) {
    char *testName = cases[i].testName;
    strcat(main, "\t(*");
...
void writeTestMainHTestCaseDllImports(char *contents, TestFileList *testFiles) {
  ...
  for (int fileIndex = 0; fileIndex < testFiles->size; fileIndex++) {
    ...
    writeTestMainHTestCaseDllImportsForSpecificFile(
        contents, file->numTestCases, file->cases);
  }
...
void writeTestMainHTestCaseDllImportsForSpecificFile(char *contents,
                                                     int numTests,
                                                     TestCase *cases) {
  for (int i = 0; i < numTests; i++) {
    char *testName = cases[i].testName;
