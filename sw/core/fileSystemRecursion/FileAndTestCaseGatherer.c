    ...
    TestFileList* testFiles, SourceFileList* sourceFiles, ObjectFileList* tempObjectFiles, int previousStepFailed, char* basePath)
{
   ...
   while ((fileOrSubDirectory = readdir(basePathDirectory)) != NULL)
   {
      ...
      addToListOrEnterSubDirectoryForRecursion(
          testFiles, sourceFiles, tempObjectFiles, previousStepFailed, basePath, fileOrSubDirectory, fileOrSubDirectoryFullPath);
   }
...
void addToListOrEnterSubDirectoryForRecursion(
    TestFileList* testFiles, SourceFileList* sourceFiles, ObjectFileList* tempObjectFiles, int previousStepFailed, char* basePath,
    struct dirent* fileOrSubDirectory, char* fileOrSubDirectoryFullPath)
{
