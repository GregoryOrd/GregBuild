...
bool isTestFile(const struct dirent *fileOrSubDirectory) {
  ...
  bool result =
      (strncmp(lower, "test", 4) == 0 &&
       (strstr(lower, ".c") != NULL || strstr(lower, ".cpp") != NULL));
  return result;
}

bool isSourceFile(const struct dirent *fileOrSubDirectory) {
  char lower[WINDOWS_MAX_PATH_LENGTH];
  bool result = (strncmp(fileOrSubDirectory->d_name, "test", 4) != 0 &&
                 (strstr(fileOrSubDirectory->d_name, ".c") != NULL ||
                  strstr(fileOrSubDirectory->d_name, ".cpp") != NULL));
  return result;
}
...
bool isTestCaseDefinition(char *line) {
  ...
  LineAnalysisResults *results =
      (LineAnalysisResults *)malloc(sizeof(LineAnalysisResults));
  ...
  bool isTestCase = results->correctStartOfLine && results->correctSpaces &&
                    results->correctBrackets && !results->hasSpecialCharacters;
...
void determineResults(LineAnalysisResults *results, LineMetrics *metrics,
  ...
  results->correctSpaces =
      results->singleSpaceBetweenBoolAndTestName && metrics->numSpaces == 1;

  results->correctBracketCount =
      metrics->numLeftBrackets == 1 && metrics->numRightBrackets == 1;
  results->correctBracketPosition =
      metrics->leftBracketIndex == metrics->expectedLeftBracketIndex &&
      metrics->rightBracketIndex == metrics->expectedRightBracketIndex;
  results->correctBrackets =
      results->correctBracketCount && results->correctBracketPosition;

  if (results->correctStartOfLine && results->correctSpaces &&
      results->correctBrackets) {
...
int testNameEndOffset(char *testName) {
  if (theCurlyBraceIsOnTheSameLineAsTheTestName(testName,
                                                strlen(testName) - 1)) {
...
bool isSpecialCharacter(char c) {
  ...
  if (isCarriageReturn || isNewLine || isNumber || isUpperCaseLetter ||
      isLowerCaseLetter || isSpace || isLeftCurrlyBracket || isParenthesis) {
    return false;
  }
  return true;
}