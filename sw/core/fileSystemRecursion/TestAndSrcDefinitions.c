...
bool isSourceFile(struct dirent* fileOrSubDirectory)
{
   char lower[WINDOWS_MAX_PATH_LENGTH];
   bool result =
       (strncmp(fileOrSubDirectory->d_name, "test", 4) != 0 &&
        (strstr(fileOrSubDirectory->d_name, ".c") != NULL || strstr(fileOrSubDirectory->d_name, ".cpp") != NULL));
   return result;
}
...
void determineResults(LineAnalysisResults* results, LineMetrics* metrics, char* line)
{
   ...
   results->correctBracketPosition =
       metrics->leftBracketIndex == metrics->expectedLeftBracketIndex && metrics->rightBracketIndex == metrics->expectedRightBracketIndex;
