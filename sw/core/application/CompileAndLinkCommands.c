...
int compileIntoTempObjectFiles(TestFileList *testFiles,
  ...
  populateArgsFor_compileIntoTempObjectFiles(tempObjectFiles, gccArgs, mvArgs,
...
void populateArgsFor_compileIntoTempObjectFiles(ObjectFileList *tempObjectFiles,
                                                ArgList *gccArgs,
                                                ArgList *mvArgs,
  ...
  getArgsForSourceFiles(tempObjectFiles, &argIndex, sourceFiles, gccArgs,
                        mvArgs);
}
...
int createTestMainExecutableFromProjectDllAndGregTestDll(
  ...
  char *const argv[] = {gcc,
                        ...
                        TEMP_TEST_MAIN_C,
                        "-L./",
                        TEMP_TEST_PROJECT_DLL,
                        LIB_GREG_TEST_DLL,
                        NULL};
  return forkAndRunChildProcess(gcc, argv);
}

int compileObjectFilesIntoProjectExecutable(TestFileList *testFiles,
                                            SourceFileList *sourceFiles,
                                            ObjectFileList *tempObjectFiles,
                                            int previousStepFailed,
                                            char *basePath) {
  exitIfPreviousStepFailed(previousStepFailed);
...
void getArgsForTestFiles(ObjectFileList *tempObjectFiles, int *argIndex,
                         TestFileList *testFiles, ArgList *gccArgs,
                         ArgList *mvArgs) {
  ...
  while (testFileIndex < testFiles->size) {
    ...
    determineObjectFileName(objectFileName,
                            testFiles->files[testFileIndex].name);
    addTempObjectFileToList(tempObjectFiles, objectFileName, false);
    gccArgs->args[*argIndex + gccFileArgOffset] =
        testFiles->files[testFileIndex].name;
    mvArgs->args[*argIndex + mvFileArgOffset] =
        (char *)malloc(strlen(objectFileName) * sizeof(char));
...
void getArgsForSourceFiles(ObjectFileList *tempObjectFiles, int *argIndex,
                           SourceFileList *sourceFiles, ArgList *gccArgs,
                           ArgList *mvArgs) {
  char *objectFileName = (char *)malloc(WINDOWS_MAX_PATH_LENGTH * sizeof(char));
  int gccFileArgOffset = 2;
  int mvFileArgOffset = 1;
  ...
  while (sourceFileIndex < sourceFiles->size) {
    clearString(objectFileName);
    determineObjectFileName(objectFileName,
                            sourceFiles->files[sourceFileIndex].name);
...
void addTempObjectFileToList(ObjectFileList *list, char *filename,
  ...
  list->files = (ObjectFile *)realloc(list->files,
                                      ((list->size + 1) * sizeof(ObjectFile)));
