...
void processPlugins(LinkedList* buildSequence, PluginList* list, LinkedList* pluginHModules, LinkedList* commandLineOptions)
{
   for (int i = 0; i < list->size; i++)
   {
      ...
      insert_ll(buildSequence, beforeLoadingTestAndSourceFilesStep, BUILD_SEQUENCE_STEP_TYPE, 0);
      insert_ll(commandLineOptions, beforeLoadingTestAndSourceFilesStep->option, COMMAND_LINE_OPTION_TYPE, 0);
   }
}
